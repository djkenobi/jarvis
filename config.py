...
class Config:
    ...
    # Protects the whole app behind a username + password login. Because the
    # app is exposed on the public internet over HTTPS, leave this ON.
    ...
    # Set the session cookie's Secure flag. MUST be true in production (HTTPS).
    # Keep false only for local HTTP testing/development.
    JARVIS_COOKIE_SECURE = _env_bool("JARVIS_COOKIE_SECURE", False)
    # Secret used to sign the session cookie. If left empty, a random secret is
    # generated on first run and persisted to SESSION_SECRET_FILE. Set it to a
    # fixed value in .env to keep sessions stable across restarts.
    SESSION_SECRET = os.getenv("SESSION_SECRET", "")
    SESSION_SECRET_FILE = os.getenv(
        "SESSION_SECRET_FILE", str(BASE_DIR / ".session_secret")
    )
