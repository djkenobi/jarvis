...
# ---------- 4. default .env ----------
# The installer generates the config files itself so it never depends on
# .env.example being uploaded (hidden dotfiles are often dropped in transfer).

# 4a. Write .env.example if it's missing (a documented template).
ENV_EXAMPLE="$APP_DIR/.env.example"
if [[ ! -f "$ENV_EXAMPLE" ]]; then
  info "Creating $ENV_EXAMPLE template"
  cat > "$ENV_EXAMPLE" <<'TEMPLATE'
# Jarvis AI Assistant — configuration template.
# Copy to `.env` and edit, or just let install.sh generate a working `.env`.
JARVIS_HOST=127.0.0.1
JARVIS_PORT=8000
OLLAMA_URL=http://localhost:11434
OLLAMA_MODEL=llama3.2
OLLAMA_USE_MOCK=false
...
JARVIS_PASSWORD=change_me_please
JARVIS_COOKIE_SECURE=true
# SESSION_SECRET=
TEMPLATE
fi

# 4b. Generate the real .env (only if it doesn't already exist).
...
if [[ -f "$APP_DIR/.env" ]]; then
  ...
  if [[ -n "${JARVIS_PASSWORD:-}" ]]; then
    ...
    # NOTE: `|| true` is required because `head` closes the pipe early, sending
    # SIGPIPE to `tr`, which under `set -o pipefail` would abort the script.
    GEN_PASSWORD="$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 18 || true)"
  fi
  info "Generating $APP_DIR/.env with defaults"
  cat > "$APP_DIR/.env" <<EOF
# Jarvis AI Assistant — generated by install.sh. Edit and restart the service.
JARVIS_HOST=127.0.0.1
JARVIS_PORT=8000
OLLAMA_URL=${OLLAMA_URL}
OLLAMA_MODEL=${MODEL}
OLLAMA_USE_MOCK=${OLLAMA_USE_MOCK:-false}
ASSISTANT_NAME=Jarvis
TTS_ENGINE=edge-tts
TTS_VOICE=en-GB-RyanNeural
TTS_SPEED=+0%
...
ADMIN_PIN=
AUTH_ENABLED=true
JARVIS_USERNAME=${AUTH_USERNAME}
JARVIS_PASSWORD=${GEN_PASSWORD}
JARVIS_COOKIE_SECURE=true
EOF
  # Protect the file — it contains the login password.
