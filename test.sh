...
# Test credentials for the live server checks.
export AUTH_ENABLED=true JARVIS_USERNAME=admin JARVIS_PASSWORD=testpass JARVIS_COOKIE_SECURE=false
...
if [[ "${1:-}" == "live" ]]; then
  ...
  echo -n "login: "
  curl -s -c /tmp/jar_cookie.txt -X POST "http://127.0.0.1:$PORT/api/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"username":"admin","password":"testpass"}' | python -c "import sys,json; d=json.load(sys.stdin); print('authed=', d.get('authed'))" || bad "login failed"

  echo -n "chat (with cookie): "
  curl -s -b /tmp/jar_cookie.txt -X POST "http://127.0.0.1:$PORT/api/chat" \
    -H "Content-Type: application/json" \
  ...
  echo -n "chat (no login, expect 401): "
  CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "http://127.0.0.1:$PORT/api/chat" \
    -H "Content-Type: application/json" -d '{"message":"Hello Jarvis"}')
  [[ "$CODE" == "401" ]] && ok "chat correctly denied (401)" || bad "expected 401, got $CODE"

  echo -n "index: "
  curl -s -o /dev/null -w "%{http_code}\n" "http://127.0.0.1:$PORT/" || bad "index endpoint failed"

  echo -n "action-denied: "
  CODE=$(curl -s -o /dev/null -w "%{http_code}" -b /tmp/jar_cookie.txt -X POST "http://127.0.0.1:$PORT/api/action" \
